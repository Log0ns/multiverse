...
let _fogCanvas = null, _fogCtx = null;
  ...
  function draw() {
    ...
    for (const torch of (_enemyCamps || [])) {
      const sx = torch.x * camera.zoom + camera.x + width / 2;
      const sy = torch.y * camera.zoom + camera.y + height / 2;
      const flicker = 0.6 + 0.4 * Math.sin(time + torch.x * 3.7);
      const r = (3 + flicker * 2) * camera.zoom;
      const grad = ctx.createRadialGradient(sx, sy, 0, sx, sy, r * 3);
      grad.addColorStop(0, 'rgba(220,50,20,' + (0.7 * flicker) + ')');
      grad.addColorStop(0.5, 'rgba(180,30,10,' + (0.3 * flicker) + ')');
      ...
      ctx.fillStyle = 'rgba(255,80,20,' + (0.8 * flicker) + ')';
      ctx.fill();
    }
    ctx.restore();
  }
