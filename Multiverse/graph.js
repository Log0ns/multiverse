// graph.js â€” Hybrid layout: fixed radial chapters + force-directed notes
const Graph = (() => {
  let canvas, ctx, width, height;
  let nodes = [], edges = [], clusters = [];
let _enemyCamps = null, _enemyCampsDay = 0;
let _fogCanvas = null, _fogCtx = null;
  ...
  function draw() {
    ...
    // Fog of war overlay
    if (!_fogCanvas || _fogCanvas.width !== width || _fogCanvas.height !== height) {
    ...
    fctx.fillStyle = 'rgba(0,0,0,0.82)';
    fctx.fillRect(0, 0, width, height);
    fctx.globalCompositeOperation = 'destination-out';
    for (const n of nodes) {
      if (n.type === 'note' || (n.type === 'chapter' && n.active)) {
        const sx = n.x * camera.zoom + camera.x + width / 2;
        const sy = n.y * camera.zoom + camera.y + height / 2;
        const r = (n.type === 'note' ? 120 : 70) * camera.zoom;
    ...
    // Let un-noted chapter nodes show through
    fctx.globalCompositeOperation = 'destination-out';
    for (const n of nodes) {
      if (n.type === 'chapter' && !n.active) {
        const sx = n.x * camera.zoom + camera.x + width / 2;
        const sy = n.y * camera.zoom + camera.y + height / 2;
        const r = (n.radius + 2) * camera.zoom;
    ...
    // Clear fog around hovered/selected node labels
    const reveal = hoveredNode || selectedNode;
    if (reveal) {
      const sx = reveal.x * camera.zoom + camera.x + width / 2;
      const sy = reveal.y * camera.zoom + camera.y + height / 2;
    ...
    // Enemy torches in darkness
    if (!_enemyCamps || _enemyCamps.camps.length === 0 || _enemyCampsDay !== Math.floor(Date.now() / 86400000)) {
      _enemyCampsDay = Math.floor(Date.now() / 86400000);
      const darkNodes = nodes.filter(n => n.type === 'chapter' && !n.active);
      const litNodes = nodes.filter(n => n.type === 'note' || (n.type === 'chapter' && n.active));
      const seed = _enemyCampsDay;
      const campCount = Math.min(8, Math.max(3, Math.floor(darkNodes.length / 80)));
      _enemyCamps = { camps: [], scouts: [] };
      if (darkNodes.length === 0) { /* no enemies if all chapters noted */ }
      else {
      // Static camps (clusters of torches)
      for (let i = 0; i < campCount; i++) {
        const idx = ((seed * 2654435761 + i * 40503) >>> 0) % darkNodes.length;
        const base = darkNodes[idx];
        const torches = 3 + (((seed + i) * 13) % 3);
        const camp = { x: base.x, y: base.y, torches: [] };
        for (let t = 0; t < torches; t++) {
          const angle = (t / torches) * Math.PI * 2 + (seed % 7);
          const dist = 5 + (t * 3);
          camp.torches.push({ ox: Math.cos(angle) * dist, oy: Math.sin(angle) * dist });
        }
        _enemyCamps.camps.push(camp);
      }
      // Scouts â€” some patrol between camps, some approach lit areas
      const scoutCount = Math.min(12, campCount * 2);
      for (let i = 0; i < scoutCount; i++) {
        const fromCamp = _enemyCamps.camps[i % campCount];
        let tx, ty, type;
        if (i < scoutCount / 2 && litNodes.length > 0) {
          // Lurker - orbits edge of a lit node
          const targetIdx = ((seed * 7 + i * 31) >>> 0) % litNodes.length;
          const target = litNodes[targetIdx];
          const lightR = target.type === 'note' ? 110 : 60;
          _enemyCamps.scouts.push({ type: 'lurker', cx: target.x, cy: target.y, radius: lightR, speed: 0.02 + (((seed + i) * 11) % 10) / 500, phase: (i * 1.3) });
        } else {
          // Patrol to another camp
          const toCamp = _enemyCamps.camps[((i + 1) * 3) % campCount];
          tx = toCamp.x;
          ty = toCamp.y;
          _enemyCamps.scouts.push({ type: 'patrol', fx: fromCamp.x, fy: fromCamp.y, tx: tx, ty: ty, speed: 0.03 + (((seed + i) * 17) % 10) / 200, phase: (i * 0.4) });
        }
      }
      }
    }
    ...
    ctx.save();
    // Draw static camp torches
    for (const camp of _enemyCamps.camps) {
      for (const t of camp.torches) {
        const wx = camp.x + t.ox;
        const wy = camp.y + t.oy;
        const sx = wx * camera.zoom + camera.x + width / 2;
        const sy = wy * camera.zoom + camera.y + height / 2;
        const flicker = 0.85 + 0.15 * Math.sin(time * 12 + wx * 7.3) * Math.sin(time * 7 + wy * 5.1);
        const r = (5 + flicker) * camera.zoom;
        const grad = ctx.createRadialGradient(sx, sy - r * 0.5, 0, sx, sy, r * 3);
        grad.addColorStop(0, 'rgba(255,200,50,' + flicker + ')');
        grad.addColorStop(0.3, 'rgba(255,80,10,0.7)');
        grad.addColorStop(0.7, 'rgba(180,20,0,0.2)');
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.arc(sx, sy - r * 0.3, r * 0.4, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(255,240,100,' + flicker + ')';
        ctx.fill();
      }
    }
        // Draw moving scouts
    for (const s of _enemyCamps.scouts) {
      let sx, sy;
      if (s.type === 'lurker') {
        const angle = time * s.speed + s.phase;
        const wx = s.cx + Math.cos(angle) * s.radius;
        const wy = s.cy + Math.sin(angle) * s.radius;
        sx = wx * camera.zoom + camera.x + width / 2;
        sy = wy * camera.zoom + camera.y + height / 2;
      } else {
        const t = ((time * s.speed + s.phase) % 2) / 2;
        const ping = t < 0.5 ? t * 2 : 2 - t * 2;
        const wx = s.fx + (s.tx - s.fx) * ping;
        const wy = s.fy + (s.ty - s.fy) * ping;
        sx = wx * camera.zoom + camera.x + width / 2;
        sy = wy * camera.zoom + camera.y + height / 2;
      }
      const flicker = 0.85 + 0.15 * Math.sin(time * 14 + s.phase * 9.3) * Math.sin(time * 9 + s.phase * 6.7);
      const r = 4 * camera.zoom;
      const grad = ctx.createRadialGradient(sx, sy - r * 0.4, 0, sx, sy, r * 2.5);
      grad.addColorStop(0, 'rgba(255,180,40,' + flicker + ')');
      grad.addColorStop(0.3, 'rgba(255,60,10,0.6)');
      grad.addColorStop(0.7, 'rgba(150,15,0,0.15)');
      ...
      ctx.fill();
      ctx.beginPath();
      ctx.arc(sx, sy - r * 0.2, r * 0.3, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(255,230,90,' + flicker + ')';
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  function onTouchStart(e) {
    ...
    if (e.touches.length === 2) {
      ...
      touchState.startX = mx;
      touchState.startY = my;
      touchState.lastMidX = mx;
      touchState.lastMidY = my;
      return;
    }
  ...
  function onTouchMove(e) {
    ...
    if (touchState.type === 'pinch' && e.touches.length === 2) {
      ...
      const my = (e.touches[0].clientY + e.touches[1].clientY) / 2;
      // Pan first
      const panDx = mx - touchState.lastMidX;
      const panDy = my - touchState.lastMidY;
      camera.x += panDx;
      camera.y += panDy;
      // Then zoom around current midpoint
      const newZoom = Math.max(0.1, Math.min(5, camera.zoom * (dist / touchState.lastDist)));
      const rect = canvas.getBoundingClientRect();
    ...
    if (touchState.type === 'pinch') {
      // Transitioned from pinch to single finger â€” reset to avoid jump
      touchState.type = 'single';
