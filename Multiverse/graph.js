...
const Graph = (() => {
  ...
  let animId = null;
  let onNodeClick = null, onNodeHover = null, onLink = null;
  ...
  function onMouseDown(e) {
    ...
    if (node) {
      ...
      if (edgeHit && selectedNode && (edgeHit.source.id === selectedNode.id || edgeHit.target.id === selectedNode.id)) {
        ...
        animateTo(otherNode.x, otherNode.y, () => { selectedNode = otherNode; if (onNodeClick) onNodeClick(selectedNode); });
        return;
      }
  ...
  function onMouseUp(e) {
    ...
    if (dragNode) {
      ...
      dragNode = null;
    } else if (panStart) { canvas.style.cursor = 'grab'; panStart = null; }
  }
  ...
  return { init, updateDynamicNodes, focusOnChapter, focusOnNode, getSelectedNode, getCamera, nodes: () => nodes };
})();
