...
let _fogCanvas = null, _fogCtx = null;
  ...
  function draw() {
    ...
    if (!_enemyCamps || _enemyCamps.camps.length === 0 || _enemyCampsDay !== Math.floor(Date.now() / 86400000)) {
      ...
      // Scouts — some patrol between camps, some approach lit areas
      ...
      for (let i = 0; i < scoutCount; i++) {
        ...
        _enemyCamps.scouts.push({ fx: fromCamp.x, fy: fromCamp.y, tx: tx, ty: ty, speed: 0.3 + (((seed + i) * 17) % 10) / 20, phase: (i * 0.4) });
      }
      }
    }
    ...
    for (const camp of _enemyCamps.camps) {
      for (const t of camp.torches) {
        ...
        const flicker = 0.6 + 0.4 * Math.sin(time * 5 + wx * 3.7);
        const r = (4 + flicker * 2) * camera.zoom;
        const grad = ctx.createRadialGradient(sx, sy, 0, sx, sy, r * 4);
        grad.addColorStop(0, 'rgba(255,60,20,' + (0.9 * flicker) + ')');
        grad.addColorStop(0.4, 'rgba(200,30,10,' + (0.4 * flicker) + ')');
        ...
        ctx.fillStyle = 'rgba(255,100,30,' + (0.9 * flicker) + ')';
        ctx.fill();
      }
    }
    // Draw moving scouts
    for (const s of _enemyCamps.scouts) {
      const t = ((time * s.speed + s.phase) % 2) / 2; // 0→1→0 ping-pong
      const ping = t < 0.5 ? t * 2 : 2 - t * 2;
      const wx = s.fx + (s.tx - s.fx) * ping;
      const wy = s.fy + (s.ty - s.fy) * ping;
      const sx = wx * camera.zoom + camera.x + width / 2;
      const sy = wy * camera.zoom + camera.y + height / 2;
      const flicker = 0.5 + 0.5 * Math.sin(time * 6 + s.phase * 10);
      const r = (3 + flicker * 1.5) * camera.zoom;
      const grad = ctx.createRadialGradient(sx, sy, 0, sx, sy, r * 3);
      grad.addColorStop(0, 'rgba(255,40,10,' + (0.8 * flicker) + ')');
      grad.addColorStop(0.5, 'rgba(180,20,5,' + (0.3 * flicker) + ')');
      ...
      ctx.fillStyle = 'rgba(255,80,20,' + (0.7 * flicker) + ')';
      ctx.fill();
    }
    ctx.restore();
  }
