...
let _fogCanvas = null, _fogCtx = null;
  ...
  function draw() {
    ...
    if (!_enemyCamps || _enemyCampsDay !== Math.floor(Date.now() / 86400000)) {
      ...
      const count = Math.min(8, Math.max(3, Math.floor(darkNodes.length / 80)));
      _enemyCamps = [];
      for (let i = 0; i < count; i++) {
        const idx = ((seed * 2654435761 + i * 40503) >>> 0) % darkNodes.length;
        const base = darkNodes[idx];
        const torches = 2 + (((seed + i) * 13) % 3);
        for (let t = 0; t < torches; t++) {
          const angle = (t / torches) * Math.PI * 2 + (seed % 7);
          const dist = 6 + (t * 4);
          _enemyCamps.push({ x: base.x + Math.cos(angle) * dist, y: base.y + Math.sin(angle) * dist });
        }
      }
    }
    const time = Date.now() / 200;
    ctx.save();
    for (const torch of (_enemyCamps || [])) {
      const drift = time * 0.3;
      const dx = Math.sin(drift + torch.x * 0.5) * 8;
      const dy = Math.cos(drift + torch.y * 0.7) * 8;
      const sx = (torch.x + dx) * camera.zoom + camera.x + width / 2;
      const sy = (torch.y + dy) * camera.zoom + camera.y + height / 2;
      const flicker = 0.6 + 0.4 * Math.sin(time + torch.x * 3.7);
      const r = (5 + flicker * 3) * camera.zoom;
      const grad = ctx.createRadialGradient(sx, sy, 0, sx, sy, r * 4);
      grad.addColorStop(0, 'rgba(255,60,20,' + (0.9 * flicker) + ')');
      grad.addColorStop(0.4, 'rgba(200,30,10,' + (0.5 * flicker) + ')');
