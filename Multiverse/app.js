...
function renderProfile() {
  ...
  if (user) {
    body.innerHTML = '<img class="pp-avatar" src="' + (user.photoURL || '') + '" alt="">' +
      ...
      '<button class="btn btn-secondary" id="pp-signout" style="width:100%;">Sign Out</button>';
...
function updateBiblePanel() {
  ...
  let html = '<div class="dp-header"><button class="dp-nav dp-prev">&#9664;</button><h3 style="margin:0;flex:1;text-align:center;">' + selectedBook + " " + ch + '</h3><button class="dp-nav dp-next">&#9654;</button><button class="dp-nav dp-mem-tab" title="Memorize">&#9733;</button>' + (window.innerWidth > 768 ? '<button class="dp-nav dp-split-open" title="Split View">&#9871;</button>' : '') + '<span class="dp-toggle">&#9660;</span></div>';
...
function openSplitViewFromBible(book, chapter) {
  ...
  const bData = BIBLE_DATA[book] && BIBLE_DATA[book][String(chapter)];
  if (bData) {
    let bhtml = '<h3>' + book + ' ' + chapter + '</h3>';
  ...
  function renderModal(fromStore) {
    ...
    function applySplit(on) {
      if (on && window.innerWidth > 768) {
        ...
        function renderSplitBible() {
          if (!splitBook || !splitChapter) return;
          const bData = BIBLE_DATA[splitBook] && BIBLE_DATA[splitBook][String(splitChapter)];
          ...
          bhtml += '<h3 style="margin:0;flex:1;text-align:center;">' + splitBook + ' ' + splitChapter + '</h3>';
