...
function updateBiblePanel() {
  ...
  let html = '<div class="dp-header"><button class="dp-nav dp-prev">&#9664;</button><h3 style="margin:0;flex:1;text-align:center;">' + selectedBook + " " + ch + '</h3><button class="dp-nav dp-next">&#9654;</button><span class="dp-toggle">&#9660;</span></div>';
  ...
  detailPanel.querySelector(".dp-header").addEventListener("click", (e) => {
    if (e.target.closest(".dp-nav")) return;
  ...
  verseEls.forEach(el => {
    el.addEventListener("click", () => {
      ...
      if (el.style.background) {
        ...
        verseEls.forEach(v => v.style.background = "");
        el.style.background = "#3b82f644";
        bibleWordSelection = [text];
      }
      updateBibleWordBar();
    });
  });
...
function updateBibleWordBar() {
  ...
  let html = '<div class="selected-text">"' + preview + '"</div>';
  ...
  if (existing) {
    ...
    bar.querySelector("#bible-word-create").addEventListener("click", () => {
      const verseEl = detailPanel.querySelector(".bible-verse[style]");
      const verseNum = verseEl ? verseEl.dataset.verse : "";
...
function showEditPanel(id) {
  ...
  let bibleScrollPos = 0;
  const dpBody = detailPanel.querySelector(".dp-body");
  if (dpBody) bibleScrollPos = dpBody.scrollTop;
  ...
  function renderModal(fromStore) {
    ...
    html += '<div class="edit-toolbar"><button type="button" class="fmt-btn" data-cmd="underline" title="Underline"><u>U</u></button><button type="button" class="fmt-btn" data-cmd="highlight" title="Highlight"><mark>H</mark></button></div>';
    ...
    modal.querySelector(".edit-save-btn").addEventListener("click", () => {
      ...
      refreshAll();
      const dpb2 = detailPanel.querySelector(".dp-body");
      if (dpb2) setTimeout(() => { dpb2.scrollTop = bibleScrollPos; }, 0);
    });
    function closeModal() {
      modal.classList.remove("active"); activeEditId = null; activeEditRender = null;
      const dpb = detailPanel.querySelector(".dp-body");
      if (dpb) setTimeout(() => { dpb.scrollTop = bibleScrollPos; }, 0);
    }
...
setInterval(updateGraphAddNotePos, 16);
...
function handleNodeClick(node) {
  if (!node) { graphAddNoteBtn.style.display = "none"; graphAddNoteNode = null; return; }
