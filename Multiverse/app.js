...
async function openSplitViewFromBible(book, chapter) {
  ...
  if (createBtn) {
    createBtn.addEventListener("click", () => {
      const newNote = Store.createNote(book + ' ' + chapter, "", chKey);
      modal.classList.remove("active", "split-view");
      refreshAll();
      localStorage.setItem('split-chapter-override', book + '_' + chapter);
    ...
    function applySplit(on) {
      if (on && window.innerWidth > 768) {
        ...
        async function renderSplitBible() {
          ...
          if (createBtn) {
            createBtn.addEventListener('click', () => {
              const newNote = Store.createNote(splitBook + ' ' + splitChapter, '', chKey);
...
function showGraphAddNote(node) {
  ...
  graphAddNoteBtn.onclick = () => {
    const note = Store.createNote(node.id.replace('_', ' '), "", node.id);
