...
function openSplitViewFromBible(book, chapter) {
  ...
  const savedWidth = localStorage.getItem('edit-split-width') || '40';
          ...
          splitBible.querySelectorAll('.split-verse').forEach(el => {
            el.addEventListener('click', () => {
              ...
              if (curText) {
                ...
                const sep = html.length === 0 ? '' : html.trimEnd().endsWith('<br>') ? '<br>' : '<br><br>';
                curText.innerHTML = html + sep + '[' + verseRef + '] ' + el.dataset.text;
              }
