...
function updateBiblePanel() {
  ...
  function updateWordHighlights() {
    wordEls.forEach((w, i) => {
      w.style.background = selectedIndices.includes(i) ? "#3b82f644" : "";
    });
  }
  function handleWordClick(el) {
    ...
    if (selectedIndices.includes(idx)) {
      ...
      // Keep only consecutive run containing the clicked word
      const runs = [];
      let run = [selectedIndices[0]];
      for (let i = 1; i < selectedIndices.length; i++) {
        if (selectedIndices[i] === selectedIndices[i - 1] + 1) { run.push(selectedIndices[i]); }
        else { runs.push(run); run = [selectedIndices[i]]; }
      }
      runs.push(run);
      const matching = runs.find(r => r.includes(idx)) || [idx];
      selectedIndices = matching;
    }
    updateWordHighlights();
    bibleWordSelection = selectedIndices.map(i => wordEls[i].textContent.replace(/[^a-zA-Z0-9' -]/g, ""));
    updateBibleWordBar();
  }
  wordEls.forEach(el => {
    el.addEventListener("click", (e) => { e.preventDefault(); handleWordClick(el); });
  });
