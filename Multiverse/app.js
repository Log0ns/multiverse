...
async function init() {
  Auth.init({
    ...
    onDataReceived: handleCloudData
  });
  Graph.init(canvas, {
...
function setupProfile() {
  ...
  btn.addEventListener("click", () => { panel.classList.toggle("active"); renderProfile(); });
...
function renderProfile() {
  ...
  const esvNotice = '<div style="margin-top:16px;padding-top:12px;border-top:1px solid #374151;font-size:0.63rem;color:#6b7280;line-height:1.4;">Scripture quotations are from the ESV&reg; Bible (The Holy Bible, English Standard Version&reg;), &copy; 2001 by Crossway, a publishing ministry of Good News Publishers. Used by permission. All rights reserved.</div>';
  if (user) {
    body.innerHTML = '<img class="pp-avatar" src="' + (user.photoURL || '') + '" alt="">' +
      '<div class="pp-name">' + (user.displayName || 'User') + '</div>' +
      '<div class="pp-email">' + (user.email || '') + '</div>' +
      '<div class="pp-sync">&#10003; Syncing to cloud</div>' +
      '<button class="btn btn-secondary" id="pp-signout" style="width:100%;">Sign Out</button>' + esvNotice;
    body.querySelector("#pp-signout").addEventListener("click", () => { Auth.signOut(); });
  } else {
    body.innerHTML = '<div style="color:#9ca3af;margin-bottom:12px;">Sign in to sync your progress across devices.</div>' +
      '<button class="btn" id="pp-signin" style="width:100%;display:flex;align-items:center;justify-content:center;gap:8px;"><svg width="16" height="16" viewBox="0 0 48 48"><path fill="#4285F4" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#34A853" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#EA4335" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg>Sign in with Google</button>' + esvNotice;
    body.querySelector("#pp-signin").addEventListener("click", () => { Auth.signIn(); });
  }
}
...
async function handleAuthStateChange(user) {
  ...
  const btn = document.getElementById("btn-profile");
  if (user) {
    btn.textContent = user.displayName ? user.displayName.split(' ')[0] : 'Profile';
    // Load cloud data first to check for conflicts
    const cloud = await Auth.loadFromCloud();
    const localData = Store.getData();
    const localHasData = localData.notes.length > 0;
    const cloudHasData = cloud && cloud.data && cloud.data.notes && cloud.data.notes.length > 0;
    if (cloudHasData && localHasData) {
      // Compare — skip dialog if identical
      if (JSON.stringify(cloud.data.notes) === JSON.stringify(localData.notes)) return;
      showCloudConflictPanel(cloud, localData);
    } else if (cloudHasData) {
      // Only cloud has data — load it
      Store.loadFromCloud(cloud.data, cloud.positions);
      refreshAll();
    } else if (localHasData) {
      // Only local has data — push to cloud
      const positions = localStorage.getItem('bible-study-positions');
      Auth.saveToCloud(localData, positions);
    }
  } else {
...
function showCloudConflictPanel(cloud, localData) {
  const existing = document.getElementById('cloud-conflict-panel');
  if (existing) existing.remove();
  const panel = document.createElement('div');
  panel.id = 'cloud-conflict-panel';
  panel.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#1f2937;border:1px solid #374151;border-radius:12px;padding:24px;z-index:10000;max-width:360px;width:90%;color:#e5e7eb;font-size:14px;';
  panel.innerHTML = '<div style="margin-bottom:16px;font-size:16px;font-weight:600;">Cloud Sync Conflict</div>'
    + '<div style="margin-bottom:16px;">Both this device and your cloud account have data:</div>'
    + '<div style="margin-bottom:8px;">This device: <strong>' + localData.notes.length + '</strong> notes</div>'
    + '<div style="margin-bottom:16px;">Cloud: <strong>' + cloud.data.notes.length + '</strong> notes</div>'
    + '<div style="display:flex;gap:8px;">'
    + '<button id="cloud-use-local" style="flex:1;padding:10px;border:none;border-radius:8px;background:#2563eb;color:#fff;cursor:pointer;">Keep Device</button>'
    + '<button id="cloud-use-cloud" style="flex:1;padding:10px;border:none;border-radius:8px;background:#374151;color:#e5e7eb;cursor:pointer;">Keep Cloud</button>'
    + '</div>';
  document.body.appendChild(panel);
  panel.querySelector('#cloud-use-local').addEventListener('click', () => {
    const positions = localStorage.getItem('bible-study-positions');
    Auth.saveToCloud(localData, positions);
    panel.remove();
  });
  panel.querySelector('#cloud-use-cloud').addEventListener('click', () => {
    Store.loadFromCloud(cloud.data, cloud.positions);
    refreshAll();
    panel.remove();
  });
...
function handleCloudData(cloudData, positions) {
  Store.loadFromCloud(cloudData, positions);
  refreshAll();
}
...
function selectBook(book, skipChapter) {
  selectedBook = book;
  buildBookNav();
  buildChapterNav();
  if (!skipChapter) selectChapter(1);
}
...
let _biblePanelRequestId = 0;
async function updateBiblePanel() {
  if (!selectedChapterKey) { detailPanel.innerHTML = ""; detailPanel.style.display = "none"; detailPanel.classList.remove("active"); return; }
  const requestId = ++_biblePanelRequestId;
  const ch = String(selectedChapter);
  const chData = await BibleAPI.getChapter(selectedBook, selectedChapter);
  ...
  let html = '<div class="dp-header"><button class="dp-nav dp-prev">&#9664;</button><h3 style="margin:0;flex:1;text-align:center;">' + selectedBook + " " + ch + ' <span style="font-size:0.65rem;color:#6b7280;font-weight:normal;">(' + translation + ')</span></h3><button class="dp-nav dp-next">&#9654;</button><button class="dp-nav dp-mem-tab" title="Memorize">&#9733;</button>' + (window.innerWidth > 768 ? '<button class="dp-nav dp-split-open" title="Split View">&#9871;</button>' : '') + '<span class="dp-toggle">&#9660;</span></div>';
  ...
  if (translation === 'ESV') html += '<p style="font-size:0.65rem;color:#6b7280;margin-top:8px;">(ESV)</p>';
  html += '</div>';
  detailPanel.innerHTML = html;
  detailPanel.querySelector(".dp-header").addEventListener("click", (e) => {
    if (e.target.closest(".dp-nav") || e.target.closest(".dp-mem-tab") || e.target.closest(".dp-split-open")) return;
  ...
  const splitOpenBtn = detailPanel.querySelector(".dp-split-open");
  if (splitOpenBtn) splitOpenBtn.addEventListener("click", () => {
    openSplitViewFromBible(selectedBook, selectedChapter);
  });
  ...
  detailPanel.querySelector(".dp-prev").addEventListener("click", () => {
    ...
    if (selectedChapter > 1) { selectChapter(selectedChapter - 1); }
      ...
      if (idx > 0) { selectBook(BOOK_NAMES[idx - 1], true); selectChapter(CHAPTER_COUNTS[BOOK_NAMES[idx - 1]]); }
    }
  });
  detailPanel.querySelector(".dp-next").addEventListener("click", () => {
    ...
    if (selectedChapter < CHAPTER_COUNTS[selectedBook]) { selectChapter(selectedChapter + 1); }
      ...
      const idx = BOOK_NAMES.indexOf(selectedBook);
      if (idx < BOOK_NAMES.length - 1) { selectBook(BOOK_NAMES[idx + 1], true); selectChapter(1); }
    }
  });
  ...
  // Note buttons at top of bible panel
  detailPanel.querySelectorAll(".dp-note-btn").forEach(btn => {
...
function updateBibleWordBar() {
  ...
  if (attachSel) {
    attachSel.addEventListener("change", () => {
      ...
      if (!note) return;
      const existing = (note.text || '').replace(/(<br\s*\/?>|\s|&nbsp;)*$/gi, '');
      if (existing.length === 0) {
        Store.updateNote(noteId, { text: '[' + verseRef + '] ' + text });
      } else {
        Store.updateNote(noteId, { text: existing + '<br><br>[' + verseRef + '] ' + text });
      }
...
// --- Open Split View from Bible Panel ---
async function openSplitViewFromBible(book, chapter) {
  const modal = document.getElementById("edit-modal");
  const chKey = book + "_" + chapter;
  const chNotes = Store.getNotesForChapter(chKey);

  localStorage.setItem('edit-split-view', '1');

  let html = '<div class="modal-header"><h2>Select a Note</h2><button class="modal-close-btn">&times;</button></div>';
  html += '<div class="split-container"><div class="modal-body" style="padding:12px 16px;">';
  html += '<div style="margin-bottom:12px;">';
  if (chNotes.length > 0) {
    html += '<label style="margin-bottom:6px;">Notes for ' + book + ' ' + chapter + ':</label>';
    for (const cn of chNotes) {
      html += '<div class="split-pick-note" data-nid="' + cn.id + '" style="padding:8px;margin:4px 0;background:#1f2937;border-radius:4px;cursor:pointer;border-left:3px solid ' + cn.color + ';font-size:0.8rem;">' + cn.title + '</div>';
    }
  } else {
    html += '<em style="color:#6b7280;font-size:0.8rem;">No notes for ' + book + ' ' + chapter + '</em>';
  }
  html += '<button class="btn split-create-new" style="margin-top:8px;font-size:0.75rem;padding:5px 12px;">+ Create Note for ' + book + ' ' + chapter + '</button>';
  html += '</div></div>';
  html += '<div class="split-divider"></div><div class="split-bible"></div></div>';
  html += '<div class="btn-row" style="padding:10px 16px;border-top:1px solid #374151;"><button class="btn btn-secondary modal-close-btn2">Cancel</button></div>';

  modal.innerHTML = html;
  modal.classList.add("active", "split-view");

  // Load bible content on right
  const splitBible = modal.querySelector(".split-bible");
  const savedWidth = Math.min(Math.max(parseInt(localStorage.getItem('edit-split-width')) || 40, 20), 60);
  splitBible.style.width = savedWidth + '%';
  const bData = await BibleAPI.getChapter(book, chapter);
  if (bData && bData.length > 0) {
    let bhtml = '<h3>' + book + ' ' + chapter + ' <span style="font-size:0.65rem;color:#6b7280;font-weight:normal;">(' + (bData._translation || 'WEB') + ')</span></h3>';
    for (const v of bData) { bhtml += '<p><sup style="color:#6b7280;margin-right:4px;">' + v.verse + '</sup>' + v.text + '</p>'; }
    if (bData._translation === 'ESV') bhtml += '<p style="font-size:0.65rem;color:#6b7280;margin-top:8px;">(ESV)</p>';
    splitBible.innerHTML = bhtml;
  }

  // Handlers
  modal.querySelector(".modal-close-btn").addEventListener("click", () => { modal.classList.remove("active", "split-view"); });
  modal.querySelector(".modal-close-btn2").addEventListener("click", () => { modal.classList.remove("active", "split-view"); });

  modal.querySelectorAll(".split-pick-note").forEach(el => {
    el.addEventListener("click", () => {
      const nid = parseInt(el.dataset.nid);
      modal.classList.remove("active", "split-view");
      localStorage.setItem('split-chapter-override', book + '_' + chapter);
      showEditPanel(nid);
    });
  });

  const createBtn = modal.querySelector(".split-create-new");
  if (createBtn) {
    createBtn.addEventListener("click", () => {
      const newNote = Store.createNote("New Note", "", chKey);
      modal.classList.remove("active", "split-view");
      refreshAll();
      localStorage.setItem('split-chapter-override', book + '_' + chapter);
      showEditPanel(newNote.id);
    });
  }
}
...
function showEditPanel(id) {
  ...
  function renderModal(fromStore) {
    ...
    const splitPref = window.innerWidth > 768 && localStorage.getItem('edit-split-view') !== '0';
    let html = '<div class="modal-header"><h2 style="color:' + note.color + ';flex:1;">' + note.title.replace(/</g, "&lt;") + '</h2>' + (window.innerWidth > 768 ? '<button class="split-toggle-btn" title="Split view" style="background:none;border:none;color:' + (splitPref ? '#60a5fa' : '#6b7280') + ';cursor:pointer;font-size:0.9rem;margin-right:12px;">&#9776;</button>' : '') + '<button class="modal-close-btn">&times;</button></div>';
    html += '<div class="split-container"><div class="modal-body" style="padding:12px 16px;">';
    ...
    html += '</select></div>';
    html += '<div id="edit-text-' + id + '" class="edit-content" contenteditable="true"></div>';
    ...
    html += '</div></details>';
    html += '</div><div class="split-divider"></div><div class="split-bible"></div></div>';
    ...
    html += '</div>';
    modal.innerHTML = html;
    const editTextEl = document.getElementById('edit-text-' + id);
    if (editTextEl) editTextEl.innerHTML = note.text || '';
    ...
    modal.querySelectorAll(".chip-ch-link").forEach(el => {
      el.addEventListener("click", () => {
        ...
        const ch = parseInt(parts.pop());
        const book = parts.join(" ");
        if (modal.classList.contains("split-view") && modal._splitNav) {
          modal._splitNav(book, ch);
        } else {
          modal.classList.remove("active"); activeEditId = null; activeEditRender = null;
          selectBook(book, true);
          selectChapter(ch);
    ...
    // Split view
    const splitBible = modal.querySelector(".split-bible");
    const splitDivider = modal.querySelector(".split-divider");
    const splitToggle = modal.querySelector(".split-toggle-btn");
    function applySplit(on) {
      if (on && window.innerWidth > 768) {
        modal.classList.add("split-view");
        const savedWidth = Math.min(Math.max(parseInt(localStorage.getItem('edit-split-width')) || 40, 20), 60);
        splitBible.style.width = savedWidth + '%';
        // Load bible content for first linked chapter
        let splitBook = null, splitChapter = null;
        const splitOverride = localStorage.getItem('split-chapter-override');
        if (splitOverride) {
          const oparts = splitOverride.split('_');
          splitChapter = parseInt(oparts.pop());
          splitBook = oparts.join(' ');
          localStorage.removeItem('split-chapter-override');
        } else if (note.chapters.length > 0) {
          const chKey = note.chapters[0];
          const cparts = chKey.split('_');
          splitChapter = parseInt(cparts.pop());
          splitBook = cparts.join(' ');
        } else {
          splitBook = selectedBook;
          splitChapter = selectedChapter;
        }
        const splitScrollPositions = {};
        async function renderSplitBible() {
          if (!splitBook || !splitChapter) return;
          const bData = await BibleAPI.getChapter(splitBook, splitChapter);
          const chKey = splitBook + '_' + splitChapter;
          let bhtml = '<div class="split-header" style="position:sticky;top:-12px;background:#111827;z-index:2;padding:20px 12px 8px;margin:-12px -12px 0;border-bottom:1px solid #374151;">';
          bhtml += '<div style="display:flex;align-items:center;justify-content:space-between;">';
          bhtml += '<button class="split-prev" style="background:none;border:none;color:#9ca3af;cursor:pointer;font-size:1rem;">&#9664;</button>';
          bhtml += '<h3 style="margin:0;flex:1;text-align:center;">' + splitBook + ' ' + splitChapter + ' <span style="font-size:0.65rem;color:#6b7280;font-weight:normal;">(' + (bData._translation || 'WEB') + ')</span></h3>';
          bhtml += '<button class="split-next" style="background:none;border:none;color:#9ca3af;cursor:pointer;font-size:1rem;">&#9654;</button>';
          bhtml += '</div>';
          const chNotes = Store.getNotesForChapter(chKey);
          bhtml += '<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px;">';
          for (const cn of chNotes) {
            bhtml += '<div class="split-note-item" data-nid="' + cn.id + '" style="font-size:0.68rem;padding:2px 6px;background:#1f2937;border:1px solid ' + cn.color + ';color:' + cn.color + ';border-radius:3px;cursor:pointer;">' + cn.title + '</div>';
          }
          bhtml += '<button class="btn split-create-note" style="font-size:0.65rem;padding:2px 6px;">+</button>';
          bhtml += '</div>';
          bhtml += '</div>';
          if (bData) {
            for (const v of bData) {
              bhtml += '<p class="split-verse" data-verse="' + v.verse + '" data-text="' + v.text.replace(/"/g, '&quot;') + '" style="cursor:pointer;border-radius:4px;padding:4px 2px;"><sup style="color:#6b7280;margin-right:4px;">' + v.verse + '</sup>' + v.text + '</p>';
            }
            if (bData._translation === 'ESV') bhtml += '<p style="font-size:0.65rem;color:#6b7280;margin-top:8px;">(ESV)</p>';
          }
          splitBible.innerHTML = bhtml;
          // Restore scroll position
          splitBible.scrollTop = splitScrollPositions[chKey] || 0;
          // Save scroll position on scroll
          splitBible.onscroll = () => { splitScrollPositions[chKey] = splitBible.scrollTop; };
          // Nav handlers
          splitBible.querySelector('.split-prev').addEventListener('click', () => {
            if (splitChapter > 1) { splitChapter--; }
            else {
              const idx = BOOK_NAMES.indexOf(splitBook);
              if (idx > 0) { splitBook = BOOK_NAMES[idx - 1]; splitChapter = CHAPTER_COUNTS[splitBook]; }
            }
            renderSplitBible();
          });
          splitBible.querySelector('.split-next').addEventListener('click', () => {
            if (splitChapter < CHAPTER_COUNTS[splitBook]) { splitChapter++; }
            else {
              const idx = BOOK_NAMES.indexOf(splitBook);
              if (idx < BOOK_NAMES.length - 1) { splitBook = BOOK_NAMES[idx + 1]; splitChapter = 1; }
            }
            renderSplitBible();
          });
          // Note switching (stays on current bible chapter)
          splitBible.querySelectorAll('.split-note-item').forEach(el => {
            el.addEventListener('click', () => {
              const nid = parseInt(el.dataset.nid);
              const curTitle = document.getElementById('edit-title-' + id).value.trim();
              const curText = document.getElementById('edit-text-' + id).innerHTML;
              if (curTitle) Store.updateNote(id, { title: curTitle, text: curText, chapters: pendingChapters, linkedNotes: pendingLinked });
              localStorage.setItem('split-chapter-override', splitBook + '_' + splitChapter);
              modal.classList.remove('active'); activeEditId = null; activeEditRender = null;
              showEditPanel(nid);
            });
          });
          // Create note for this chapter
          const createBtn = splitBible.querySelector('.split-create-note');
          if (createBtn) {
            createBtn.addEventListener('click', () => {
              const newNote = Store.createNote('New Note', '', chKey);
              const curTitle = document.getElementById('edit-title-' + id);
              const curText = document.getElementById('edit-text-' + id);
              if (curTitle && curTitle.value.trim()) Store.updateNote(id, { title: curTitle.value.trim(), text: curText.innerHTML, chapters: pendingChapters, linkedNotes: pendingLinked });
              localStorage.setItem('split-chapter-override', splitBook + '_' + splitChapter);
              modal.classList.remove('active'); activeEditId = null; activeEditRender = null;
              refreshAll();
              showEditPanel(newNote.id);
          ...
          // Verse click to attach to current note
          splitBible.querySelectorAll('.split-verse').forEach(el => {
            el.addEventListener('click', () => {
              const verseRef = splitBook + ' ' + splitChapter + ':' + el.dataset.verse;
              const curText = document.getElementById('edit-text-' + id);
              if (curText) {
                const html = curText.innerHTML.replace(/(<br\s*\/?>|\s|&nbsp;)*$/gi, '');
                if (html.length === 0) {
                  curText.innerHTML = '[' + verseRef + '] ' + el.dataset.text;
                } else {
                  curText.innerHTML = html + '<br><br>[' + verseRef + '] ' + el.dataset.text;
                }
              }
              if (!pendingChapters.includes(chKey)) pendingChapters.push(chKey);
        ...
        renderSplitBible();
        modal._splitNav = function(book, ch) { splitBook = book; splitChapter = ch; renderSplitBible(); };
      } else {
        modal.classList.remove("split-view");
      }
    }
    applySplit(splitPref);
    if (splitToggle) splitToggle.addEventListener("click", () => {
      const newPref = !modal.classList.contains("split-view");
      localStorage.setItem('edit-split-view', newPref ? '1' : '0');
      splitToggle.style.color = newPref ? '#60a5fa' : '#6b7280';
      applySplit(newPref);
    });
    // Draggable divider
    splitDivider.addEventListener("mousedown", (e) => {
      ...
      const startW = splitBible.offsetWidth;
      const totalW = modal.offsetWidth;
      function onMove(ev) {
        const newW = Math.max(150, Math.min(totalW - 200, startW - (ev.clientX - startX)));
        const pct = Math.round(newW / totalW * 100);
        splitBible.style.width = pct + '%';
        localStorage.setItem('edit-split-width', pct);
      }
  ...
  const splitPref = window.innerWidth > 768 && localStorage.getItem('edit-split-view') !== '0';
  if (splitPref) modal.classList.add("split-view");
  else modal.classList.remove("split-view");
  renderModal();
  modal.classList.add("active");
}
...
statusBar.addEventListener("click", async () => {
  const result = await Store.connectFolder();
  if (!result) return;
  if (result.conflict) {
    showSyncConflictPanel(result);
  } else {
    refreshAll();
    updateStatus();
  }
...
function showSyncConflictPanel(result) {
  const existing = document.getElementById('sync-conflict-panel');
  if (existing) existing.remove();
  const panel = document.createElement('div');
  panel.id = 'sync-conflict-panel';
  panel.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#1f2937;border:1px solid #374151;border-radius:12px;padding:24px;z-index:10000;max-width:360px;width:90%;color:#e5e7eb;font-size:14px;';
  panel.innerHTML = `<div style="margin-bottom:16px;font-size:16px;font-weight:600;">Sync Conflict</div>`
    + `<div style="margin-bottom:16px;">Both the app and the folder have data:</div>`
    + `<div style="margin-bottom:8px;">App: <strong>${result.appNotes}</strong> notes</div>`
    + `<div style="margin-bottom:16px;">Folder: <strong>${result.folderNotes}</strong> notes</div>`
    + `<div style="display:flex;gap:8px;">`
    + `<button id="sync-use-app" style="flex:1;padding:10px;border:none;border-radius:8px;background:#2563eb;color:#fff;cursor:pointer;">Keep App</button>`
    + `<button id="sync-use-folder" style="flex:1;padding:10px;border:none;border-radius:8px;background:#374151;color:#e5e7eb;cursor:pointer;">Keep Folder</button>`
    + `</div>`;
  document.body.appendChild(panel);
  panel.querySelector('#sync-use-app').addEventListener('click', async () => {
    await Store.resolveConflict('app', result.folderData);
    panel.remove();
    refreshAll();
    updateStatus();
  });
  panel.querySelector('#sync-use-folder').addEventListener('click', async () => {
    await Store.resolveConflict('folder', result.folderData);
    panel.remove();
    refreshAll();
    updateStatus();
  });
