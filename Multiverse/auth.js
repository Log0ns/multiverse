...
const Auth = (() => {
  ...
  let _ignoreNextSnapshot = true; // Ignore the initial snapshot on subscribe
  ...
  function listenToCloud() {
    ...
    unsubSnapshot = ref.onSnapshot((doc) => {
      ...
      // Ignore snapshots triggered by our own writes (within 5s)
      if (remote.updatedAt && remote.updatedAt.toMillis) {
        const serverTime = remote.updatedAt.toMillis();
        if (Math.abs(serverTime - _lastSaveTimestamp) < 5000) return;
      } else {
        // Pending local write (serverTimestamp not yet resolved) — skip
        return;
      }
      try {
