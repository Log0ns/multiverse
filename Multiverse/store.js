...
const Store = (() => {
  let dirHandle = null;
  let data = { notes: [], nextId: 1, memorization: [] };
  ...
  function deleteNote(id) {
    ...
    // Remove text links pointing to deleted note
    data.notes.forEach(n => {
      if (n.text && n.text.includes('data-note-id="' + id + '"')) {
        const div = document.createElement('div');
        div.innerHTML = n.text;
        div.querySelectorAll('.note-text-link[data-note-id="' + id + '"]').forEach(el => {
          el.replaceWith(el.textContent);
        });
        n.text = div.innerHTML;
      }
    });
    save();
  }
  ...
  // --- Memorization ---
  // Levels: 0=new, 1=learning, 2=familiar, 3=known, 4=memorized
  // Intervals (days): 0=0, 1=1, 2=3, 3=7, 4=30
  const MEM_INTERVALS = [0, 1, 3, 7, 30];

  function addToMemorize(book, chapter, verse, text) {
    const key = book + '_' + chapter + '_' + verse;
    if (data.memorization.find(m => m.key === key)) return;
    data.memorization.push({
      key, book, chapter: String(chapter), verse: String(verse), text,
      level: 0, streak: 0, lastReviewed: null, nextReview: Date.now()
    });
    save();
  }

  function removeFromMemorize(key) {
    data.memorization = data.memorization.filter(m => m.key !== key);
    save();
  }

  function updateMemorization(key, result) {
    const m = data.memorization.find(v => v.key === key);
    if (!m) return;
    if (result === 'got') {
      m.level = Math.min(m.level + 1, 4);
      m.streak++;
    } else if (result === 'almost') {
      m.streak = 0;
    } else {
      m.level = Math.max(m.level - 1, 0);
      m.streak = 0;
    }
    m.lastReviewed = Date.now();
    const days = MEM_INTERVALS[m.level];
    m.nextReview = Date.now() + days * 24 * 60 * 60 * 1000;
    save();
  }

  function getMemorizationForChapter(book, chapter) {
    return data.memorization.filter(m => m.book === book && m.chapter === String(chapter));
  }

  function getAllMemorization() { return data.memorization; }

  function getDueMemorization() {
    const now = Date.now();
    return data.memorization.filter(m => m.nextReview <= now);
  }
  ...
  function save() {
    ...
    if (!_cloudSyncPaused && typeof Auth !== 'undefined' && Auth.isSignedIn()) {
      const positions = localStorage.getItem('bible-study-positions');
      Auth.saveToCloud(data, positions);
    }
  }

  function loadFromCloud(cloudData, positions) {
    if (!cloudData || !cloudData.notes) return;
    _cloudSyncPaused = true;
    data = cloudData;
    if (!data.memorization) data.memorization = [];
    localStorage.setItem('bible-study-data', JSON.stringify(data));
    if (positions) localStorage.setItem('bible-study-positions', positions);
  ...
  async function connectFolder() {
    try {
      ...
      try {
        ...
        if (parsed && parsed.notes) folderData = parsed;
        else if (parsed && parsed.subjects) folderData = migrateOldData(parsed);
      } catch (e) { /* no file in folder */ }
      const appHasData = data.notes.length > 0;
      const folderHasData = folderData && folderData.notes && folderData.notes.length > 0;
      if (folderHasData && appHasData) {
        // Both have data — return conflict info for UI to resolve
        return { conflict: true, appNotes: data.notes.length, folderNotes: folderData.notes.length, folderData };
      } else if (folderHasData) {
        // Only folder has data — load it
        data = folderData;
        if (!data.memorization) data.memorization = [];
        localStorage.setItem('bible-study-data', JSON.stringify(data));
        return { conflict: false };
      } else {
        // Only app has data or both empty — push to folder
        if (appHasData) await saveToFiles();
        return { conflict: false };
      }
    } catch (e) {
  ...
  async function resolveConflict(choice, folderData) {
    if (choice === 'app') {
      // Overwrite folder with app data
      await saveToFiles();
    } else {
      // Load folder data into app
      data = folderData;
      if (!data.memorization) data.memorization = [];
      localStorage.setItem('bible-study-data', JSON.stringify(data));
    }
  ...
  async function loadFromFiles() {
    ...
    try {
      ...
      if (parsed && parsed.notes) {
        ...
        if (!data.memorization) data.memorization = [];
        return true;
      } else if (parsed && parsed.subjects) {
        data = migrateOldData(parsed);
        if (!data.memorization) data.memorization = [];
        return true;
      }
  ...
  async function saveDirHandle() {
    ...
    try {
      ...
      tx.objectStore('handles').put(dirHandle, 'syncFolder');
  ...
  function openDB() {
    return new Promise((resolve, reject) => {
      const req = indexedDB.open('multiverse-fs', 1);
  ...
  return {
    ...
    addToMemorize, removeFromMemorize, updateMemorization,
    getMemorizationForChapter, getAllMemorization, getDueMemorization,
    connectFolder, resolveConflict, reconnectFolder, isFileSystemConnected, exportAsZip, save, load,
    loadFromCloud, getData
  };
})();
